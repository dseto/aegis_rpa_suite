...
**Per-tier verified recovery (2026-07-16, fixed — was the `_effect_confirmed` gap):** `_attempt_deterministic_click_recovery`'s internal `_effect_confirmed` used to call `_click_effect_signals_changed` directly against a **single baseline captured once, before the original click** — bypassing `_verify_action_effect`'s overlay caveat. Two consequences, both confirmed live: (1) each tier has its own *preparatory gesture* that mutates the very page the shared baseline describes — tier 2.5 presses Escape (which closes a panel **by construction**), tier 2.75 repositions `.cdk-overlay-pane` and dispatches a synthetic click — so a tier approved on the delta **its own gesture manufactured**, whether or not the click landed; (2) tier 2.9 confirmed on churn produced two tiers earlier (the exact Fimm `st_007` signature: ambiguous `fallback_selectors` via `.first`, panel never closed, step logged `HEALED`). Fixed by `_tier_baseline(used_selector)` — a **fresh snapshot per tier**, captured *after* that tier's preparatory gesture and immediately before its physical click (same selector on both sides) — plus routing `_effect_confirmed` through `_verify_action_effect`, with `panel_closed_confirmed` derived from the **same metric the caveat uses** (a real reduction in the snapshot's `overlays` count — *not* `_OPEN_PANEL_SELECTOR`, whose different vocabulary would approve trivially inside a Bootstrap modal and make recovery impossible inside a mat-dialog). A permanent overlay now never closes → the tier never approves → false `HEALED` is dead *by construction*, not by timing. The exception path (`_handle_click_failure`, `before_snapshot=None`) is still uncovered — see the backlog below.

**Rubber-stamp fix + value-aware 4th signal (2026-07-16).** `_verify_generic_effect` recaptured the `after` snapshot **without a selector** while callers captured the `before` **with** one — so `siblingClassFingerprint` compared `"btn active|…"` against `""` and the 4th signal fired **always**: the verifier structurally **could not reject**, silently vacuuming every "verified" tier (A3 ambiguous candidate, `.first` fallback, recorded coordinate, cognitive). A `verify_rejected=0/0` telemetry reading is consistent with this bug, not necessarily with innocence. Fixed by tagging each snapshot with `fingerprintSelector` and recapturing the `after` with the **same** selector as the `before`. **Critical coupling:** fixing the rubber stamp alone drove the reference bot to **0/3** — a click on `input[type=range]` (`st_043`/`st_045`) changes only the **value**: url, domSize, overlays and className are all identical, so the sensor was blind to it and the old `HEALED=3` was **two false-HEALEDs papering over that blindness**. The 4th signal therefore also captures `aria-valuenow` and `value`. Result (gate, headed, cognitive ON, 3 runs): **3/3 SUCCESS, 55 SUCCESS / 1 HEALED** vs. the previous **1-2/3, 53 / 3** — the sliders now resolve as `identity` on the first click (no escalation), and the one remaining `HEALED` is `st_054` (closed Shadow DOM), which genuinely needs the coordinate tier. Baseline: `.specs/plans/portal-segura.baseline-001.md` Section 3.

**📋 Open findings — read before touching this chain: `.specs/backlog-achados-falso-sucesso.pending.md`.** Trackable backlog of every false-success finding still open, from the audit plus three independent reviews (plan-critic Sonnet, plan-critic Fable, cross-vendor Forge/GPT-5.5) and a direction reassessment. Nothing in it is implemented. Highlights: the **exception path** logs raw `SUCCESS` (not even `HEALED`, so zero Sensor F1) with **no verification at all** — the most frequent recovery path; `select_option_resilient`'s coordinate/live-geometry tiers close without a post-condition; `fill_human_like`'s identity route and the fill-side recovery tiers are unverified. It also records a structural limit: generic signals prove *"no evidence of non-effect"*, never *"my click caused the effect"* — ambient DOM churn inside the polling window closes a false `SUCCESS` at the identity level, and **no baseline refinement fixes it** (the real fix is Phase 2's recorded `expected_effect`). And a trap worth internalizing: **do not** harden the identity path with the overlay caveat — it would reject legitimate panel-closing clicks, trigger recovery, and **re-click an action that already worked** (a false negative with a physical side effect is worse than a false-positive status).

**Retry-loop laundering seam (2026-07-15, fixed — distinct from the `_effect_confirmed` gap above):** `click_resilient`'s two `_finalize_click_success` call sites (empty-locators branch and candidate branch) used to call it *unguarded*. When `_finalize_click_success` exhausts CLICK_NO_EFFECT recovery and `_handle_unrecoverable_click` **raises** the terminal decision, that raise fell into the generic `except Exception as e` of the outer `for attempt in range(1,3)` loop, which treated it as an ordinary physical-click failure and **retried**. Since `click_effect_before_snapshot` is captured once outside the loop, attempt 2 compared against a stale baseline; an incidental DOM delta accumulated across the failed attempts (spinner/toast) crossed the ±2 tolerance and the step closed as a false `SUCCESS` (identity, not `HEALED` → not even registered for review). Reproduced deterministically live (overlay intercepting a click, action never performed, yet `SUCCESS`). Fixed by wrapping both `_finalize_click_success` calls in `_ClickTerminalFailure` — the same terminal-propagation contract the ENABLE_TIMEOUT path already used — so a finalized CLICK_NO_EFFECT decision propagates out instead of being retried. Before/after proven with a fault-injection harness (`event.isTrusted`-gated target so synthetic-dispatch recovery tiers can't cheat; churn anchored to the `[RETRY 2]` Escape to isolate this seam from the `_effect_confirmed` gap); 118 runner integration tests green.

**Two refinements this guard *requires* (don't ship it without them).** (1) **Attempt-aware terminality** — guarding *every* attempt made `for attempt in range(1, 3)` dead code for the whole CLICK_NO_EFFECT path, silently dropping a real capability: attempt 2 does `Escape` → `wait_for_selector(state="visible")` → **re-enumerates candidates** → de-prioritizes dead `href="#"` anchors → verifies the ambiguous candidate (A3), none of which the deterministic chain does (it only ever tries `.first`). Failure it would cost: slow render, attempt 1 sees only the dead anchor, the real target renders 300ms later. So `_finalize_click_success` takes `attempt` (default `2` = terminal, fail-safe for unknown callers): on `attempt < 2` it raises CLICK_NO_EFFECT as **retryable** (nothing logged, no decision taken) and the call-site re-raises it bare; only on `attempt == 2` does it delegate to `_handle_unrecoverable_click` and wrap in `_ClickTerminalFailure`. (2) **Per-attempt baseline** — `click_effect_before_snapshot` is captured **inside** the attempt loop, right before that attempt's click, never once outside it. Restoring the retry without this re-opens the exact laundering the guard exists to kill: attempt 2 would compare against the pre-attempt-1 baseline, and churn accumulated by the failed attempts crosses the ±2 tolerance. The two changes are inseparable — verified by the same harness (scenario B stays an honest failure; a legitimate heal still closes `HEALED` + `needs_review`).

### Unified Target Descriptor — anchor geometry tier (2026-07-20, PR #2)

Inspired by UiPath's unified targeting: every recorded `click`/`fill` event carries an optional redundant descriptor alongside the primary selector — `anchor` (nearest stable label/text + relative geometry) and `expected_effect` (the delta the action actually caused during recording). Both are captured by `JS_MINIMAL_LISTENERS` in `recorder.py` (`computeGeometricAnchor`, `captureSnapshot`/`computeSnapshotDelta`), propagated through the Sanitizer (`build_step_from_event` → `_serialize_plan_step`), and emitted as extra kwargs (`anchor=`, `expected_effect=`, `viewport=`) on `click_resilient`/`fill_resilient`/`select_option_resilient` calls in generated bots — only when present, so old recordings/bots are unaffected.

**Anchor election** (in order): `label[for=id]` → `aria-labelledby`/`aria-label` target → ancestor/preceding `<label>` → nearest visible text node within 250px (filtered: 2-60 chars, not pure-numeric, not inside a hash-like id/class). `_resolve_via_anchor` (`runner.py`) locates the anchor live (selector, falling back to exact/fuzzy text), disambiguates multiple matches by nearest recorded bbox, computes the target point (anchor center + recorded `dx`/`dy`, scaled by current/recorded viewport ratio), and returns the element at that point if plausible against `target_description` (same loose check as `_hit_test_plausible`) or the nearest interactive element within an 80px radius otherwise. It only **resolves** — never clicks — same contract as every other tier in this doctrine.

**Wired into three runtime call sites**, each reading `self._current_anchor`/`_current_expected_effect`/`_current_viewport` (set unconditionally at the top of `click_resilient`/`fill_resilient`/`select_option_resilient` from the kwargs — every call resets the state, so nothing leaks between steps without needing `try/finally`): click's tier 2.95 (between `fallback_selectors` and recorded coordinate — allowed under `strict=True`, same as `fallback_selectors`, since it's recorded/validated data, not a guess), fill's equivalent tier, and — the highest-value integration, since Angular Material `mat-select` dropdowns are the framework's worst-measured flakiness (`st_023`/`st_026` of the Portal Segura reference bot) — a trigger-resolution tier in `select_option_resilient` between the known-selector cascade and the recorded-coordinate fallback. A resolved+clicked anchor only closes as `HEALED` (`healing_method="anchor_geometry"`) after `_verify_action_effect` confirms a real effect, using the **same selector** for the tier's before/after baseline (the anchor's own selector — passing a different literal label for the "after" capture reopens the exact rubber-stamp class of bug `_verify_generic_effect`'s `fingerprintSelector` fix already closed once).

`_verify_action_effect`'s recorded-`expected_effect` path (`_verify_recorded_expected_effect`) OR-matches against the delta actually observed during recording (`url_changed`, `dom_delta`/`overlay_delta` same-sign, `new_visible_text` present) — any single match confirms; no match falls back to the existing generic signals (never rejects solely because the specific recorded expectation didn't fire, preserving the identity path's existing tolerance).

**Validated live** against the Portal Segura reference site (not just mocked): a fresh recording captured real `anchor`+`expected_effect` (label_for/nearest_stable_text strategies, real geometry deltas), the Sanitizer→Generator pipeline produced a bot on attempt 1/15 with **zero LLM calls**, and a deliberately-broken primary selector with the correct recorded anchor was resolved and clicked by the tier in a live run, with the page genuinely transitioning to the next screen. Report: `.specs/relatorio-piloto-portal_segura_utd_pilot.md`.

**Known gap:** live validation exercised the click tier positively (broken selector, healthy anchor, real navigation confirmed) but not yet the `select_option_resilient` trigger tier specifically against a live Angular Material dropdown (the recording that produced the tested plan stopped before reaching the vehicle-data screen) — the mechanism is proven generically, not yet on the exact `st_023`/`st_026`-class target that motivated it.
