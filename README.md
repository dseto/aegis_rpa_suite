...
14. **Sensor `CLICK_NO_EFFECT` (Passo Fantasma) — detecção + correção ativa:** `click_resilient` usa `click(force=True)` no clique físico principal, que pula a checagem de actionability do Playwright — um clique em elemento coberto por overlay pode "passar" sem efeito real. Antes do clique, o runner tira um snapshot barato via `page.evaluate()` (`url`, contagem de nós DOM com tolerância ±2, contagem de overlays `.cdk-overlay-container *, [role=dialog], .modal.show`, fingerprint de classe dos irmãos diretos); depois do clique, faz polling com saída antecipada em ~100/300/800ms — qualquer sinal mudar, sai imediatamente (custo típico ~100ms). `document.activeElement` **não** entra como sinal de efeito: no engine Chromium (MS Edge) o próprio clique já move o foco, o que mascararia exatamente o caso-alvo (clique `force=True` sob overlay). Se nenhum sinal mudar até 800ms, o passo **não** é mais fechado como sucesso direto: o runner aciona, antes de decidir, a mesma cadeia determinística de recuperação usada pra falhas por exceção (Escape+retry → reposicionar `.cdk-overlay-pane` + clique sintético → `fallback_selectors` do item 15) — só cai pro fallback cognitivo/coordenada (bloqueado sob `strict=True`) se nenhuma camada determinística produzir efeito real. Um passo recuperado assim fecha como `HEALED` (`healing_method="click_no_effect_recovered"`) e registra `needs_review` pelo mesmo Sensor F1 do item 12, sem necessidade de flag extra. Controlado por `AEGIS_CLICK_EFFECT_SENSOR` (default `true`; `false` desativa completamente, zero `evaluate()` extra) e excluído para `validate_navigation=True` e para os seletores literais das famílias do item 9/10 (já cobertos por espera dedicada). O dedup do item 12 se aplica normalmente (só suprime enquanto o par está `needs_review`/`pending`).
