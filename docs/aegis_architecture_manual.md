...
graph TD
    ...
    C -->|dataset validado| D["4. Code Generator — LLM (aegis_sanitizer/code_generator.py)"]
...
### 4. Aegis Code Generator (Compilação via LLM)
* **Módulo:** `aegis_sanitizer/code_generator.py` — CLI: `python aegis_sanitizer/code_generator.py --project-dir projects/<slug>`.
* **Função:** Compila `bot_producao.py` + `skills_lib.py` chamando um provedor de LLM (OpenRouter, LiteLLM, ou qualquer endpoint compatível com OpenAI — configurado via `AEGIS_COGNITIVE_PROVIDER`/`AEGIS_COGNITIVE_API_KEY`/`AEGIS_COGNITIVE_MODEL`, com `AEGIS_COGNITIVE_CODER_MODEL` opcional dedicado a esta fase). Requer o `plano_execucao.json` da Fase 2 e o dataset validado da Fase 3.
* **Dois fluxos:**
  * **Geração nova:** compila o robô do zero a partir do `plano_execucao.json` e do catálogo de padrões de resiliência.
...
* **Saída:** `bot_producao.py` + `skills_lib.py` salvos em `projects/<slug>/tests/<slug_do_teste>/code/`.
...
* **Isolamento de Projetos e Proteção do Core Framework (Aegis Suite Blindado):**
  ...
  * **Separação Externa de Projetos:** Tudo o que for específico de um processo automatizado (RPA) deve ser externo ao motor. A estrutura `aegis_runner`, `aegis_blackbox`, `aegis_cockpit`, `aegis_sanitizer`, `aegis_mentor` é um motor blindado e não deve receber alterações específicas de um robô.
